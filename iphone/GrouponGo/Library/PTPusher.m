...
{
	NSLog(@"subscribing with auth point: %@", authPoint);
  ...
  if (!channel) {
	...
	channel.authPoint = [NSURL URLWithString:@"http://go.groupon.com/pusher/auth"] ;
...
- (void)webSocket:(ZTWebSocket*)webSocket didReceiveMessage:(NSString*)message
{
	...
	PTPusherEvent *event = [[PTPusherEvent alloc] initWithDictionary:[message JSONValue]];
	...
	if ([event.name isEqualToString:@"pusher:connection_established"]) {
		...
		NSLog(@"socket gave error: %@", [event description]);
	}
